...
class GossipDataParallel(Module):
    ...
    def __init__(self, module, device_ids=None, rank=None, world_size=None,
                 ...
                 eta = 0.5, momentum=0.9, lr=0.1, weight_decay = 1e-4, qgm=1,):
...
##############################################################
    ...
    def _gossip_into_receive_buffer(send_buffer, gossiper, receive_buffer,
        ...
        with gossip_lock:
            ...
            in_msg, updated_ps_weight, data_amt = gossiper.mix(flatten_tensors(send_buffer), send_buffer, ps_weight, residual=True, 
                                                              epoch=dist_config['epoch'], iteration=dist_config['iterations'])
