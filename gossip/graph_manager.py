...
class GraphManager(object):
    ...
    def __init__(self, rank, world_size, nprocs_per_node=1, local_rank=0, peers_per_itr=2):
...
class RingGraph_dynamic(GraphManager):
    ...
    def _make_graph(self):
        for rank in range(self.world_size):
            ...
            b_peer = self._rotate_backward(rank, 1)
            if rank == 0 or rank ==1:
                f3 = self._rotate_backward(rank, 2)
            if rank ==2:
                f3 = self._rotate_backward(rank, 1)
            if rank==3:
                f3 = self._rotate_forward(rank, 1)
            self._add_peers(rank, [f_peer, b_peer, f3])
